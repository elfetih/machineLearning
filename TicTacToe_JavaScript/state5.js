function MenuState(name) {
	...
	btns.push(new MenuButton("Two Player Game", 20, _yPos+100, function() {
		state.get("game").init(TWO_PLAYER);
		state.change("game");
	}));
	btns.push(new MenuButton("About Game", 20, _yPos+200, function() {
		...
		if (!activeAnim) {
			...
			if (winner && !aiMoved) {
				if (winner === true) {
					...
					winnerMsg = "The O player won";
				} else {
					winnerMsg = "The X player won";
				}
			}
...
function AboutState(name) {
	...
	var text = "Tic-tac-toe est un jeu pour deux joueurs, X et O, qui marquent à tour de rôle les espaces dans une grille de 5 × 5. Le joueur qui réussit à placer trois marques respectives dans une rangée horizontale, verticale ou diagonale gagne la partie.";
	var hastick = false;
