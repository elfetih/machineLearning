...
function AIPlayer(data) {
	...
	function evaluate() {
		var s = 0;
		s += evaluateLine(0, 1, 2, 3, 4);
		s += evaluateLine(5, 6, 7, 8, 9);
		s += evaluateLine(10, 11, 12, 13, 14);
		s += evaluateLine(15, 16, 17, 18, 19);
		s += evaluateLine(20, 21, 22, 23, 24);
		s += evaluateLine(0, 5, 10, 15, 20);
		s += evaluateLine(1, 6, 11, 16, 21);
		s += evaluateLine(2, 7, 12, 17, 22);
		s += evaluateLine(3, 8, 13, 18, 23);
		s += evaluateLine(4, 9, 14, 19, 24);
		s += evaluateLine(0, 6, 12, 18, 24);
		s += evaluateLine(4, 8, 12, 16, 20);

		return s;
	}

	function evaluateLine(idx1, idx2, idx3, idx4, idx5) {
		var s = 0;
		...
		} else if (data[idx5].equals(oppSeed)) {
			if (s < 0) {
				s *= 10;
			} else if (s > 0) {
				return 0;
	...
	var winnigPatterns = (function() {
		var wp = ["1111100000000000000000000", "0000011111000000000000000", "0000000000111110000000000", "0000000000000001111100000", 
				  "0000000000000000000011111", "1000010000100001000010000", "0100001000010000100001000", "0100001000010000100001000",
